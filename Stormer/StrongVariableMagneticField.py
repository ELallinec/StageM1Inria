...
def jacobienne(y,u):
    ...
    temp = y[0]**2 + y[1]**2
    D = u[0]*y[1]-u[1]*y[0]
    jacob [0,0] = -u[1]*y[1]*temp -2*y[0]*y[1]*D
    jacob[0,1] = (2*y[1]*u[0]-u[1]*y[0])*temp-2*y[1]**2 * D
    jacob[1,0] = (2*y[0]*u[1]-u[0]*y[1])*temp +2*y[0]**2 * D
    jacob[1,1] = -u[0]*y[0]*temp + 2*y[0]*y[1]*D

    jacob[0,3] = y[1]**2 * temp
    jacob[0,4] = -y[0]*y[1]*temp
    jacob[1,3] = -y[0]*y[1]*temp
    jacob[1,4] = y[0]**2 *temp

    jacob[3,0] = u[1]**2*temp +2*y[0]*u[1]*D
    jacob[3,1] = -u[0]*u[1]*temp +2*y[1]*u[1]*D
    jacob[4,0] = -u[0]*u[1]*temp -2*y[0]*u[0]*D
    jacob[4,1] = u[0]**2 * temp -2*y[1]*u[0]

    jacob[3,3] = -u[1]*y[1]*temp
    jacob[3,4] = (2*u[1]*y[0]-u[0]*y[1])*temp
    jacob[4,3] = (2*u[0]*y[1]-u[1]*y[0])*temp
    jacob[4,4] = -u[0]*y[0]*temp
    jacob /= temp**2
    return jacob